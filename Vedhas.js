...
/*    const btnPitchSvarita = document.getElementById('btnPitchSvarita');
   const btnPitchDirgha = document.getElementById('btnPitchDirgha'); */
...
function renderMantraTabs() {
    ...
    if (currentBookKey && currentSectionKey && allBooks[currentBookKey].sections[currentSectionKey].mantras) {
        ...
        if (mantraKeys.length > 0) {
            ...
            mantraTitleDisplay.innerHTML = '<h3>మంత్రం లేదు</h3>';
            mantraDescriptionDisplay.innerHTML = '<p>ఈ భాగంలో మంత్రం లేదు. కొత్త మంత్రాన్ని జోడించండి.</p>';
...
function preparePitches(pitchType) {
    ...
    updateStatus(`పిచ్‌లు గుర్తించబడ్డాయి. ఇప్పుడు మీరు మంత్రాన్ని సేవ్ చేయవచ్చు.`, 'info');
}
...
btnSaveMantra.addEventListener('click', () => {
    ...
    if (newKey !== currentMantraKey && allBooks[currentBookKey].sections[currentSectionKey].mantras[newKey]) {
        alert(`'${title}' పేరుతో ఒక మంత్రం ఇప్పటికే ఉంది. దయచేసి ప్రత్యేకమైన పేరును ఎంచుకోండి.`);
        return;
    }
    ...
    pitchMarks = [];
    currentMantraKey = null;
    updateStatus(`'${title}' మంత్రం విజయవంతంగా సేవ్ చేయబడింది.`, 'success');
    btnSaveMantra.textContent = 'కొత్త మంత్రాన్ని సేవ్ చేయండి ✨';
});
...
async function downloadPdfFromHtml(htmlContent, filename) {
  // Create a temporary container element for htmlContent
  const container = document.createElement('div');
  container.style.position = 'fixed';
  container.style.left = '-9999px';  // Offscreen
  container.style.width = '800px';    // PDF page width approx
  container.innerHTML = htmlContent;
  document.body.appendChild(container);

  // Use html2canvas to render container
  const canvas = await html2canvas(container, { scale: 2 });

  // Remove temporary container from the DOM
